...
utils.initData(intervalSize, noOfDomains, noOfAgentsPerDomain, minLimitOfSwarmExecuted);
 ...
 assert.callback(`How many swarms can be executed in ${utils.intervalSize}ms (${utils.deployedDomains} domains, ${utils.noOfAgentsPerDomain} agents per domain)`, (finished) => {
    tir.launch(utils.intervalSize+utils.intervalSize*0.3, () => {
        ...
        setTimeout(()=>{
            ...
            assert.true(swarmCounter>utils.minLimitOfSwarmExecuted, `Unsurpassed limit!`);
            finished();
            tir.tearDown(0);
        }, utils.intervalSize);
